...
from network_architecture.IFPN import IFPN3D
...
from Loss_functions import JILoss, FocalLoss, softmax_helper, sum_tensor
...
class Trainer(nn.Module):
    def __init__(self, fold, data_root, out_path, output_fold):
        ...
        self.seg_loss = JILoss({'batch_dice': self.batch_dice, 'smooth': 1e-5, 'do_bg': False, 'square': False})
        self.class_loss = FocalLoss(gamma=0.2, alpha=0.70, size_average=False)
        self.gender_loss = torch.nn.CrossEntropyLoss().cuda()
        self.age_loss = torch.nn.MSELoss().cuda()
        self.fp16 = False
    ...
    def run_iteration(self, data_generator, do_backprop=True, run_online_evaluation=False):
        ...
        gender_target = data_dict['gender_label']
        age_target = data_dict['age_label']
        ...
        l1 = self.class_loss(output[1], class_target.squeeze().long())
        l2 = self.gender_loss(output[2], gender_target.squeeze().long())
        l3 = self.age_loss(output[3], age_target.squeeze().long())
        l = l0 + l1 + 0.01*l2 + 0.01*l3
        if run_online_evaluation:
            self.run_online_evaluation(output[0], output[1], target, class_target)
