...
class DataLoader3D(SlimDataLoaderBase):
    ...
    def generate_train_batch(self):
        ...
        age_label = []
        case_properties = []
        class_name = pd.read_excel('/home/ubuntu/zhangyongtao/3D IFPN/class_label.xlsx',
                                   dtype=object, keep_default_na=False, na_values=[]).values
        gender_name = pd.read_excel('/home/ubuntu/zhangyongtao/3D IFPN/gender_label.xlsx',
                                   dtype=object, keep_default_na=False, na_values=[]).values
        age_name = pd.read_excel('/home/ubuntu/zhangyongtao/3D IFPN/age_label.xlsx',
                                   dtype=object, keep_default_na=False, na_values=[]).values
        ...
        for j, i in enumerate(selected_keys):
            ...
            if self.has_prev_stage:
                ...
                assert all(
                    [i == j for i, j in zip(seg_from_previous_stage.shape[1:], case_all_data.shape[1:])]), \
            ...
            for c in class_name:
                global class_zyt
                if c[0] == i:
                    class_zyt = np.array(float(c[1]))
            class_label.append(class_zyt)

            for g in gender_name:
                global gender_zyt
                if g[0] == i:
                    gender_zyt = np.array(float(g[1]))
            gender_label.append(gender_zyt)

            for a in age_name:
                global age_zyt
                if a[0] == i:
                    age_zyt = np.array(float(a[1]))
            age_label.append(age_zyt)
        ...
        gender_label = np.vstack(gender_label)
        age_label = np.vstack(age_label)

        return {'data':data, 'seg':seg, 'class_label':class_label, 'gender_label':gender_label, 'age_label':age_label, 'properties':case_properties, 'keys': selected_keys}
